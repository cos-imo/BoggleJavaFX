# Boggle - JavaFX

![Static Badge](https://img.shields.io/badge/Télécom-Projet_scolaire-purple)
![Static Badge](https://img.shields.io/badge/Langage-Java-blue?logo=data:image/svg+xml;base64,)
![Static Badge](https://img.shields.io/badge/Framework-JavaFX-blue?logo=JavaFX)

## Sommaire
1. Sommaire
2. Présentation
3. Installation
4. Organisation du dépôt et informations complémentaires

## Présentation
Projet développé dans le cadre de ma scolarité à [Télécom Nancy](https://telecomnancy.univ-lorraine.fr) (2ème année - 1er semestre)
Extrait du sujet:

> L’objectif des 2 prochaines séances de TP est de [développer] le jeu Boggle.       

La présente application est constituée d'une interface implémentée en JAvaFX

## Installation

## Installation
##### Téléchargement
###### Par SSH
```
git@github.com:cos-imo/BoggleJavaFX.git
```
###### Par HTTPS
```
https://github.com/cos-imo/BoggleJavaFX.git
```

##### Compilation
Depuis la racine du projet (dossier `BoggleJavafx`), utilisez gradlew pour installer les dépendances requises:
```
./gradlew build
```


##### Lancement
Depuis la racine du projet (dossier `BoggleJavaFX`) exécuter gradlew:
```
./gradlew run
```

## Organisation du dépôt et informations complémentaires
